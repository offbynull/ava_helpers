...
from fixed_joint_mating.mater_lcs import mater_lcs_attacher, mater_lcs_attachment_y_facing_positive_z_orienter, \
    mater_lcs_attachment_y_facing_edge_orienter, mater_lcs_creator, mater_lcs_detacher
from fixed_joint_mating.mater_lcs.mater_lcs_identifier import MATER_LCS_IDENTIFIER
from logger import error, log


def create_and_attach(doc: App.Document) -> list[App.DocumentObject] | None:
    picked = []
    ...
    for _, vert_obj, vert_name in verts:
        ...
        mater_lcs = mater_lcs_creator.run(doc, f'{face_obj.Label}_{MATER_LCS_IDENTIFIER}')
